...
spm_encode --model=$model_dir/sentence.bpe.model < $DATA/en-ro.vaccine.en > $DATA/en-ro.vaccine.mbart.spm.en_XX
spm_encode --model=$model_dir/sentence.bpe.model < $DATA/en-ro.vaccine.ro > $DATA/en-ro.vaccine.mbart.spm.ro_RO

fairseq-preprocess \
  --source-lang en_XX \
  --target-lang ro_RO \
  --testpref $DATA/en-ro.vaccine.mbart.spm \
  --destdir $DATA/vacc.mbart.spm.en-ro \
  --thresholdtgt 0 \
  --thresholdsrc 0 \
  --srcdict ${DICT} \
  --tgtdict ${DICT} \
...
CUDA_VISIBLE_DEVICES=0 fairseq-generate  $DATA/vacc.mbart.spm.en-ro\
